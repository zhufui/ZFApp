...
import cn.com.permissiongen.internal.Utils;
...
public class PermissionGen {
    ...
    @TargetApi(value = Build.VERSION_CODES.M)//这只是屏蔽了错误提示
        ...
        if (Utils.isOverMarshmallow()) {
            ...
            if (deniedPermissions.size() > 0) {
                if (object instanceof Activity) {
                    String[] pers = deniedPermissions.toArray(new String[deniedPermissions.size()]);
                    ((Activity) object).requestPermissions(pers, requestCode);
    ...
    private static void doExecuteSuccess(Object activity, int requestCode) {
        Method method = Utils.findMethodPermissionSuccessWithRequestCode(activity.getClass(), PermissionSuccess.class, requestCode);
        executeMethod(activity, method);
    }

    private static void doExecuteFail(Object activity, int requestCode) {
        Method method = Utils.findMethodPermissionSuccessWithRequestCode(activity.getClass(), PermissionFail.class, requestCode);
        executeMethod(activity, method);
    }
